...
namespace RealtimeCurve {
		...
		CPlot::CPlot(void)
	{
		...
		m_vppYAxis = 1.5f;	// 每个像素代表的Y轴值

		m_vppYAxis_Speed = 1.5f;
		ShowTextGrap = 5;	// 每隔多个格显示一个坐标值

		m_marginLeft = 50;
		m_marginRight = 37;
		m_marginTop = 31;//表格离顶部的距离
		m_marginBottom = 20;
	...
	void CPlot::AddNewPoint(float x, float y, int nLineIndex)
	{
		...
		m_axisX.SetAxisRange(x - 35.0f, x);
		// 记录X轴的范围
		m_originXLwLmt = x - 35.0f;
		m_originXUpLmt = x;
		...
		if (m_bAdjustable)
		{
			...
			m_axisY.SetAxisRange(Temp_minchange, Temp_maxchange);
			//Speed的Y轴设置
			m_axisY_Speed.SetAxisRange(Speed_minchange, Speed_maxchange);
	...
	CAxis& CPlot::GetAxisY_Speed()
	{
		return m_axisY_Speed;
	}
	...
	{
		if (flag_changeSize == true)
		{
			m_marginTop = 31;
			flag_changeSize = false;
			m_marginRight = 37;

			/*CString look;
			look.Format(_T("%d"), m_marginRight);
			MessageBox(look);*/
		}
		
		// 更新空间区域
		m_marginTop = m_marginTop -y_subtract;
		y_subtract = 0;
		m_marginRight = m_marginRight - x_subtract;
		x_subtract = 0;
		...
		Speed_Line = 100/((m_rectPlot.right - m_rectPlot.left)/175.0);

		/*CString look;
		look.Format(_T("%f"), Speed_Line);
		MessageBox(look);*/
		...
		// 图例区域
		m_rectLegend.left = m_rectTitle.right - 20;
		...
		//Speed的Y轴设置
		m_vppYAxis_Speed = (m_axisY_Speed.GetAxisRange()) / m_rectPlot.Height();
	}
	...
	void CPlot::DrawAxises(CDC* pDC)
	{
		...
		if (m_axisY.IsShow)
		{
			...
			pDC->MoveTo(m_rectPlot.right, m_rectPlot.bottom);
			pDC->LineTo(m_rectPlot.right, m_rectPlot.top);
			// 恢复画笔
			pDC->SelectObject(oldPen);
		}
	...
	void CPlot::DrawGrids(CDC* pDC)
	{
		...
		static int count_position = 0;
		static int count_X_time = 0;
		static bool flag_Time[10] = { 0 };
		...
		X_GridSize = x_gridsize;


		for (int i = 0, xi = 0;
			xi = i*this->X_GridSize + m_rectPlot.left, xi <= m_rectPlot.right;
			i++)
		{
			if (i>0) {	//与Y轴重合的不用绘制
				if (i%ShowTextGrap == 0)
					pDC->MoveTo(xi, m_rectPlot.bottom + 5);
				else
			...
			if (i%ShowTextGrap == 0 )
			{
				...
				pDC->SetTextColor(m_axisX.AxisColor);	// 设置字体颜色				
														//Min.Format(_T("%4.1f"), m_axisX.GetRangeLowerLimit() + i*m_vppXAxis*GridSize);
				Min.Format(_T("%d"), Time / 60);
				switch (i) {
				case(0):
					Sec.Format(_T("%d"), (Time - 35) % 60);
					if (flag_Time[0] == 1 && (Time - 35) % 60 != 0)
						Min.Format(_T("%d"), Time / 60 - 1);
					break;
				case(5):
					Sec.Format(_T("%d"), (Time - 30) % 60);
					if (flag_Time[1] == 1 && (Time - 30) % 60 != 0)
						Min.Format(_T("%d"), Time / 60 - 1);
					break;
				case(10):
					Sec.Format(_T("%d"), (Time - 25) % 60);
					if (flag_Time[2] == 1 && (Time - 25) % 60 != 0)
						Min.Format(_T("%d"), Time / 60 - 1);
					break;
				case(15):
					Sec.Format(_T("%d"), (Time - 20) % 60);
					if (flag_Time[3] == 1 && (Time - 20) % 60 != 0)
						Min.Format(_T("%d"), Time / 60 - 1);
					break;
				case(20):
					Sec.Format(_T("%d"), (Time - 15) % 60);
					if (flag_Time[4] == 1 && (Time - 15) % 60 != 0)
						Min.Format(_T("%d"), Time / 60 - 1);
					break;
				case(25):
					Sec.Format(_T("%d"), (Time - 10) % 60);
					if (flag_Time[5] == 1 && (Time - 10) % 60 != 0)
						Min.Format(_T("%d"), Time / 60 - 1);
					break;
				case(30):
					Sec.Format(_T("%d"), (Time - 5) % 60);
					if (flag_Time[6] == 1 && (Time - 5) % 60 != 0)
						Min.Format(_T("%d"), Time / 60 - 1);
					break;
				case(35):
					...
					if (Time % 60 == 0 && Time != 0)
					{
						flag_Time[0] = flag_Time[1] = flag_Time[2] = flag_Time[3] = flag_Time[4] = flag_Time[5] =flag_Time[6]= 1;
					}

					if (Time % 60 >= 5 && flag_Time[6] == 1)
						flag_Time[6] = 0;
					if (Time % 60 >= 10 && flag_Time[5] == 1)
						flag_Time[5] = 0;
					if (Time % 60 >= 15 && flag_Time[4] == 1)
						flag_Time[4] = 0;
					if (Time % 60 >= 20 && flag_Time[3] == 1)
						flag_Time[3] = 0;
					if (Time % 60 >= 25 && flag_Time[2] == 1)
						flag_Time[2] = 0;
					if (Time % 60 >= 30 && flag_Time[1] == 1)
						flag_Time[1] = 0;
					if (Time % 60 >= 35 && flag_Time[0] == 1)
						flag_Time[0] = 0;
				...
				g_strTime = Min + _T(":") + Sec;
				count_time += 5;
				pDC->DrawText(g_strTime, CRect(xi - m_axisX.CoorTextBoxWidth / 2,
					m_rectPlot.bottom + 6,	// +10是为了不挡住X坐标轴,突出每五格
			...
			i++)
		{
			...
			{
				if (i%ShowTextGrap == 0) {
					pDC->MoveTo(m_rectPlot.left - 8, yi);//突出有坐标的横线
				}
				else
			...
			// 显示Y坐标值
			if (i%(ShowTextGrap-4)== 0)
			{
				CFont *oldFont = pDC->SelectObject(&textFont);// 设置字体格式
				pDC->SetTextColor(m_axisY.AxisColor);	// 设置字体颜色				
														//txtStr.Format(_T("%3.2f"), m_axisY.GetRangeLowerLimit() + i*m_vppYAxis*GridSize);
				txtStr.Format(_T("%3.0f"), Temp_minchange + count_position*(Temp_maxchange - Temp_minchange) /20*1.0);
				...
				//Y轴右侧
				txtStr.Format(_T("%2.1f"), Speed_minchange + count_position*(Speed_maxchange - Speed_minchange) / 20*1.0);
				pDC->DrawText(txtStr, CRect(m_rectCtrl.left,
					yi - m_axisY.CoorTextBoxHeight / 2,
					m_rectPlot.right + 30,
					yi + m_axisY.CoorTextBoxHeight / 2),
		...
		for (int i = 0; i<GetLineCount(); i++)
		{
			...
			if (i == 1 || i>1) {//此处对应序号为1-13的曲线使用右侧参考坐标轴（即风速、风压）
				for (int k = 0; k<nLine->GetPointCount(); k++)
				{
					...
					int y = (int)(m_rectPlot.bottom - ((nLine->GetPointY(k) - m_axisY_Speed.GetRangeLowerLimit()) / (m_axisY_Speed.GetAxisRange())*m_rectPlot.Height()));
					// 将转化后的值存入数组
					nCPointArray[k].x = x;
					nCPointArray[k].y = y;
				}
	...
	void CPlot::OnLButtonDblClk(UINT nFlags, CPoint point)
	{
		...
		//Speed的Y轴坐标设置
		m_axisY_Speed.SetAxisRange(m_originYLwLmt, m_originYUpLmt);
